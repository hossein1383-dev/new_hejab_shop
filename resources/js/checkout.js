...
document.querySelector('[data-place-order]')?.addEventListener('click', async (event) => {
    ...
    payload.service_type = document.querySelector('input[name="service_type"]:checked')?.value || '1';
...
/**
 * تخمین زنده هزینه ارسال — بخش ۱۵ و ۵۲. هر بار آدرس (موجود یا تازه) عوض
 * شود، قیمت هر دو گزینه (پیشتاز/ویژه) هم‌زمان گرفته و کنار هرکدام نشان
 * داده می‌شود؛ جمع کل بر اساس گزینه‌ای که مشتری انتخاب کرده به‌روزرسانی
 * می‌شود. مبلغ نهایی واقعی همیشه توسط سرور هنگام ثبت سفارش دوباره
 * محاسبه می‌شود؛ این فقط برای نمایش زنده به کاربر است.
 */
(function () {
    ...
    let lastShippingCosts = {}; // { '1': 85000, '3': 120000 }

    function formatToman(amount) {
        return amount === 0 ? 'رایگان' : `${new Intl.NumberFormat('fa-IR').format(amount)} تومان`;
    }

    /** انیمیشن شمارش نرم عدد از مقدار قبلی به مقدار جدید (به‌جای پرش ناگهانی). */
    function animateNumberChange(el, toValue) {
        const fromText = el.textContent.replace(/[^۰-۹0-9]/g, '');
        const fromValue = fromText ? parseInt(fromText.replace(/[۰-۹]/g, (d) => '۰۱۲۳۴۵۶۷۸۹'.indexOf(d)), 10) : toValue;
        if (!Number.isFinite(fromValue) || fromValue === toValue) {
            el.textContent = formatToman(toValue);
            return;
        }
        ...
        function step(now) {
            ...
            el.textContent = formatToman(current);
    ...
    function updateTotalsForSelectedService() {
        const checkedRadio = document.querySelector('input[name="service_type"]:checked');
        const selected = checkedRadio ? checkedRadio.value : '1';
        const shippingCost = lastShippingCosts[selected];
        if (shippingCost === undefined || shippingCost === null) return;

        animateNumberChange(shippingDisplay, shippingCost);
        const subtotal = lastShippingCosts.subtotal || 0;
        const total = subtotal + shippingCost;
        if (totalDisplay) animateNumberChange(totalDisplay, total);
        if (payButton) payButton.dataset.baseTotal = total;
        if (payAmountDisplay) payAmountDisplay.textContent = `پرداخت ${formatToman(total)}`;
    }

    async function fetchPriceForServiceType(baseFields, serviceType) {
        try {
            const response = await window.apiFetch('/checkout/estimate-shipping', {
                method: 'POST',
                body: JSON.stringify({ ...baseFields, service_type: serviceType }),
            });
            const { shipping_cost: shippingCost, subtotal } = response.data;
            lastShippingCosts[serviceType] = shippingCost;
            lastShippingCosts.subtotal = subtotal;

            const el = document.querySelector(`[data-service-price="${serviceType}"]`);
            if (el) el.textContent = formatToman(shippingCost);

            updateTotalsForSelectedService();
        } catch (error) {
            const el = document.querySelector(`[data-service-price="${serviceType}"]`);
            if (el) el.textContent = 'خطا در محاسبه';
        }
    }

    async function updateShippingEstimate(baseFields) {
        servicePriceEls.forEach((el) => { el.textContent = 'در حال محاسبه...'; });

        await Promise.all([
            fetchPriceForServiceType(baseFields, 1),
            fetchPriceForServiceType(baseFields, 5),
        ]);
    ...
    document.querySelectorAll('[data-service-type-radio]').forEach((radio) => {
        radio.addEventListener('change', updateTotalsForSelectedService);
    });
